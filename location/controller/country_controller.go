...
func (c *CountryController) GetAll(e echo.Context) error {
	...
	if len(countries) == 0 {
		return config.SuccessResponse(e, nil, config.CountryNotFound)
	}

	return config.SuccessResponse(e, countries, config.GetCountrySuccess)
}
...
func (c *CountryController) GetByID(e echo.Context) error {
	...
	return config.SuccessResponse(e, country, config.GetCountrySuccess)
}
